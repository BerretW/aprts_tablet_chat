var ChatApp = {
    ...
    init: function() {
        ...
        window.removeEventListener('message', ChatApp.onMessage);
        window.addEventListener('message', ChatApp.onMessage);
    ...
    onMessage: function(event) {
        ...
        if (data.action === 'chat_registered') {
            ...
            if (ChatApp.currentRoomId === data.roomId) {
                document.getElementById('msg-container').innerHTML = '<div style="opacity:0.5; text-align:center; margin-top:20px;">Tato místnost byla smazána.</div>';
                document.getElementById('current-room-name').innerText = "Místnost smazána";
    ...
    showChatInterface: function() {
        ...
        document.getElementById('display-serial').innerText = 'ID: ' + this.mySerial.slice(-4);
    ...
    searchRooms: function() {
        ...
        let filtered = ChatApp.allRooms.filter(r => r.name.toLowerCase().includes(query));
    ...
    switchRoom: function(id, name) {
        ...
        document.getElementById('msg-container').innerHTML = '<div style="opacity:0.5; text-align:center; margin-top:20px;">Načítání zpráv...</div>';
    ...
    createRoomPrompt: function() {
        if (typeof Swal !== 'undefined') {
            ...
            if (name) System.pluginAction('chat', 'createRoom', { 
                name: name,
                serial: ChatApp.mySerial
            });
        }
    ...
    renderMessages: function(messages) {
        ...
        if(messages.length === 0) {
            container.innerHTML = '<div style="opacity:0.5; text-align:center; margin-top:20px;">Zatím žádné zprávy.</div>';
        }
    ...
    appendMessage: function(msg) {
        ...
        if(typeof time === 'string' && time.length > 16) time = time.substring(11, 16);
